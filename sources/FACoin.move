module echo::launchpad {
    ...
    public entry fun set_pending_admin(sender: &signer, new_admin: address) acquires Config {
        ...
        let config = borrow_global_mut<Config>(@echo);
    ...
    public entry fun create_fa(
        ...
        let fa_owner_obj_constructor_ref = &object::create_object(@echo);
        ...
        let registry = borrow_global_mut<Registry>(@echo);
    ...
    public fun get_admin(): address acquires Config {
        let config = borrow_global<Config>(@echo);
        config.admin_addr
    }
    ...
    public fun get_registry(): vector<Object<Metadata>> acquires Registry {
        let registry = borrow_global<Registry>(@echo);
        registry.fa_objects
    }
    ...
    fun is_admin(config: &Config, sender: address): bool {
        if (sender == config.admin_addr) {
            ...
            if (object::is_object(@echo)) {
                let obj = object::address_to_object<ObjectCore>(@echo);
    ...
    #[test(aptos_framework = @0x1, sender = @echo)]
    fun test_happy_path(
