module echo::token_marketplace {
    ...
    #[test_only]
    friend echo::test_token_marketplace;
...
#[test_only]
module echo::test_token_marketplace {
    ...
    use echo::token_marketplace::{Self, Listing};
    use echo::test_utils;
    ...
    fun test_fixed_price(
        ...
        let (listing_obj, seller_addr2) = token_marketplace::listing(listing);
        ...
        assert!(token_marketplace::price<AptosCoin>(listing) == option::some(500), 0); // The price is 500.
        assert!(object::owner(token) == object::object_address(&listing), 0); // The token is owned by the listing object. (escrowed)

        token_marketplace::purchase<AptosCoin>(purchaser, object::convert(listing));
    ...
    #[test(aptos_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x60001, location = echo::token_marketplace)]
    fun test_no_listing(
        aptos_framework: &signer,
        marketplace: &signer,
        seller: &signer,
        ...
        token_marketplace::purchase<AptosCoin>(purchaser, object::convert(dummy_object));
    }
    ...
    inline fun fixed_price_listing_with_token(
        ...
        let listing = token_marketplace::list_with_fixed_price_internal<AptosCoin>(
            seller,
